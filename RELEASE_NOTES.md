...
- 修正 Native Helper installer 與公開文件誤用錯誤 Chrome Web Store item ID，確保正式商店版 origin `jdfempgjnmdlokacfjmnpiphhghcnomb` 能連接 Helper。
...
1. 在隔離 Chrome profile 載入候選版，確認 Popup、content script、background 都是 2.19.3；重新整理測試用 X／Threads 分頁，再確認擴充功能錯誤頁。
...
5. 通過後才提交變更並建立新的 `v2.19.3` tag。既有公開 tag 不移動；Release workflow 會依新 tag 執行測試並產出套件。
...
- `dist/sun-pit-v2.19.3.zip`
- `dist/sun-pit-helper-v2.19.3-macos.zip`
- `dist/SHA256SUMS`
